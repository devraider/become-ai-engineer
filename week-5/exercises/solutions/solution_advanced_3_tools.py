...
def get_weather(location: str, unit: str = "celsius") -> Dict:
    ...
    data = weather_data.get(location.lower(), {"temp_c": 20, "condition": "unknown", "humidity": 50})
    temp = data["temp_c"]
    if unit == "fahrenheit":
        temp = temp * 9/5 + 32
...
def search_database(query: str, limit: int = 5) -> List[Dict]:
    ...
    products = [
        ...
        {"id": 5, "name": "Mechanical Keyboard", "price": 150, "category": "electronics"},
    ]
...
class ToolAgent:
    ...
    def _should_use_tool(self, question: str) -> Optional[str]:
        ...
        keywords = {
            "get_weather": ["weather", "temperature", "forecast", "hot", "cold", "rain"],
            "search_database": ["search", "find", "product", "database", "look up"],
            "calculate": ["calculate", "math", "compute", "+", "-", "*", "/", "sum", "multiply"],
        }
    ...
    def process_question(self, question: str) -> str:
        ...
        if tool_name:
            ...
            if result.get("success"):
                return f"Result from {tool_name}: {json.dumps(result['result'], indent=2)}"
            else:
